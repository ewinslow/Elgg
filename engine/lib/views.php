...
function elgg_get_viewtype() {
	return (string)_elgg_services()->views->getCurrentViewtype();
}
...
function elgg_view_exists($view, $viewtype = 'default', $recurse = true) {
	$views = _elgg_services()->views;
	$viewtype = $views->getOrCreateViewtype($viewtype);
	return $views->getView($view)->exists($viewtype, $recurse);
}
...
function elgg_views_boot() {
	$coreViewsDir = GaufretteDirectory::createLocal(dirname(dirname(__DIR__)) . "/views");
	_elgg_services()->views->registerViewsDirectory($coreViewsDir);
